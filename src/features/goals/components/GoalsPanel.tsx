  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 shadow-[var(--shadow-card)]">
        <h3 className="mb-2 text-secondary">Goals for {period}</h3>
        <p className="text-tertiary">Set targets for realized performance and track provisional progress with unrealized P&L.</p>
        <div className="mt-3 grid grid-cols-1 gap-2.5 md:grid-cols-3">
          {(Object.keys(GOAL_LABELS) as GoalType[]).map((type) => (
            <div key={type} className="space-y-2 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-2.5">
              <label className="text-label block">{GOAL_LABELS[type]}</label>
              <input
                type="number"
                min="0"
                step="0.01"
                ...
                className="text-numeric h-11 w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 text-secondary-sm outline-none focus:border-[var(--accent)]"
              ...
              <button
                ...
                className="min-h-11 w-full rounded-lg bg-[var(--accent)] px-3 py-2 text-secondary-sm text-black transition hover:brightness-110"
              >
                Save
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        {progress.map((item) => (
          <div key={item.goal.id} className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 shadow-[var(--shadow-card)]">
            <div className="mb-3 flex items-center justify-between">
              <h4 className="text-secondary-sm">{GOAL_LABELS[item.goal.type]}</h4>
              <button
                type="button"
                onClick={() => onDeleteGoal(item.goal.id)}
                className="text-tertiary-sm transition hover:text-[var(--negative)]"
              >
                Delete
              </button>
            </div>
            <p className="text-tertiary">
              ...
              <span className="text-numeric">{formatGoalValue(item.goal.type, item.goal.target, formatCurrency)}</span>
            </p>
            <p className="mt-1 text-secondary-sm">
              <span className="text-label">Realized: </span>
              <span className="text-numeric">{formatGoalValue(item.goal.type, item.current, formatCurrency)}</span>
            </p>
            <p className="text-secondary-sm">
              <span className="text-label">With Unrealized: </span>
              <span className="text-numeric">{formatGoalValue(item.goal.type, item.currentWithUnrealized, formatCurrency)}</span>
            </p>
            ...
            <div className="mt-3 space-y-2">
              <div>
                <div className="mb-1 flex items-center justify-between text-xs">
                  <span className="text-label">Realized</span>
                  <span className={`text-numeric ${statusClass(item.status)}`}>{item.progressPercent.toFixed(0)}%</span>
                </div>
              ...
              <div>
                <div className="mb-1 flex items-center justify-between text-xs">
                  <span className="text-label">Provisional</span>
                  <span className={`text-numeric ${statusClass(item.statusWithUnrealized)}`}>{item.progressPercentWithUnrealized.toFixed(0)}%</span>
                </div>
                <div className="h-2 rounded-full bg-[var(--surface-2)]">
        ...
        {progress.length === 0 && (
          <div className="rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 text-tertiary">
